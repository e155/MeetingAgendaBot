...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"👋 Привет, *{_name(user)}*!\n\n"
        f"Этот бот помогает вести митинги в Telegram-группе.\n"
        f"Вы добавляете пункты повестки обсуждения и задачи здесь, в личном диалоге с этим ботом. "
        f"В день митинга один из вас запускает митинг в группе и управляет ходом обсуждения прямо из группы: "
        f"переключает пункты, фиксирует решения и задачи. "
        f"По завершении бот публикует решения и итоговый протокол в группу.\n\n"
